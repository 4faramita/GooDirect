...
if (window.top === window) {
    ...
    addEventListener("mousedown", cleanLink, true);
    addEventListener("touchend", cleanLinkMobile, true);
}
...
function cleanLinkMobile(e) {
    let anchor = anchorMatch(e.target), ping;

    if (anchor && anchor.localName == "a") {
        ping = anchor.getAttribute("ping");
        
        if (ping && ping.indexOf("/url?") == 0) {
...
//            var a = anchorMatch(event.target);
//
//            if (a && a.localName == 'a') {
//                cleanTheLink(a);
//            }
